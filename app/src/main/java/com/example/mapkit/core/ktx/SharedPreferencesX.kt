...
fun SharedPreferences.double(
    ...
    override fun getValue(
        thisRef: Any,
        property: KProperty<*>
    ): Double = getString(key(property).camelToSnakeCase(), defaultValue.toString())?.toDouble() ?: defaultValue
...
inline fun <reified T : Enum<T>> SharedPreferences.enum(
    ...
    override fun getValue(thisRef: Any, property: KProperty<*>): T {
        return enumValueOf(getString(key(property).camelToSnakeCase(), defaultValue.name) ?: defaultValue.name)
    }
